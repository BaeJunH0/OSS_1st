...
Copilot은 분명히 생산성을 높여주는 훌륭한 도구지만, 완전히 믿고 맡기기에는 아직 한계가 있는 보조 도구다. AI가 제안하는 코드가 항상 올바르거나 최적화되어 있는 것은 아니며, 때로는 불필요하거나 비효율적인 방식으로 코드를 제시하기도 한다. 또한, 코드에 보안 취약점이 포함될 수도 있고, 오픈소스에서 학습된 코드가 그대로 포함되어 라이선스 문제가 발생할 수 있는 가능성도 있다.

따라서 Copilot이 제안하는 코드를 사용할 때는 반드시 검토와 수정이 필요하며, 실제 서비스에 적용하기 전에는 충분한 테스트가 요구된다. 특히 보안이 중요한 분야나 고성능이 필요한 시스템에서는 Copilot의 코드를 신중하게 다루어야 한다.

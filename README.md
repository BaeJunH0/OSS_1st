...
branch는 프로젝트의 주 흐름인 main 혹은 master와는 별개로 독립적인 작업을 할 수 있게 해주는 기능이다. 예를 들어 새로운 기능을 개발하거나 버그를 수정할 때, 기존 코드에 영향을 주지 않고 브랜치를 만들어 작업한 후, 모든 작업이 완료되면 다시 메인 브랜치로 병합(merge)할 수 있다. 이 과정에서 GitHub는 충돌(conflict)이 발생할 경우, 어떤 부분에서 문제가 생겼는지를 표시해주어 개발자들이 쉽게 해결할 수 있도록 돕는다.

**Pull Request**

Pull Request 기능을 통해 협업을 체계적으로 관리할 수 있다. branch에서 작업한 내용을 main branch에 반영하고 싶을 때, 단순히 병합하는 것이 아니라 먼저 Pull Request를 열어 팀원들에게 코드 변경 내용을 리뷰받을 수 있다. 이 과정은 코드 품질을 높이고, 의도치 않은 버그나 보안 문제를 사전에 방지하는 데 큰 도움이 된다. Pull Request는 리뷰, 코멘트, 승인 등의 과정을 포함할 수 있어 팀워크와 커뮤니케이션에 매우 유용하다.

**history tracking과 rollback**

프로젝트의 모든 변경 이력을 시간 순서대로 기록하는 기능인 history tracking과 실수하거나 문제가 생겼을 때 이전 상태로 되돌리는 rollback도 GitHub의 강력한 VCS 기능이다.사용자는 과거의 특정 commit 시점으로 프로젝트를 되돌릴 수 있으며, 누가 언제 어떤 변경을 가했는지 세부적으로 확인할 수 있다. 이는 실수로 코드를 삭제하거나 문제가 발생했을 때, 빠르게 복구할 수 있게 해준다. 이러한 특성 덕분에 GitHub는 실시간 백업 시스템으로서의 역할도 겸하고 있다.
